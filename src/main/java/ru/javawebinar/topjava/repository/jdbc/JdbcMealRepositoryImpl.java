...
/*    private static final RowMapper<Meal> ROW_MAPPER = (resultSet, i) -> new Meal(resultSet.getInt("id"),
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            ...
            if (namedParameterJdbcTemplate.update("UPDATE meals " +
                    ...
                    "WHERE id=:id AND user_id=:userId", mapParameter)
                    == 0) return null;
        }
    ...
    public Meal get(int id, int userId) {
        return DataAccessUtils.singleResult(
                jdbcTemplate.query("SELECT id, datetime, description, calories FROM meals WHERE id=? AND user_id=?", ROW_MAPPER, id, userId));
    }
